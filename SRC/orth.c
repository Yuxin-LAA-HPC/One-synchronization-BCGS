        ...
        int ldR, double *work, int lwork)
{
    ...
    dlacpy_(ALL, &msub, &s, Xsub, &ldXsub, Usub, &msub, 1);
    dgemm_(NOTRANS, NOTRANS, &msub, &s, &s0, &rone, Usub, &msub, worktemp,
    ...
    for (int i = 1; i < p; i++)
    {
        ...
        //dlacpy_("A", &msub, &s, Xsub, &ldXsub, Usub, &msub);
        dgemm_(NOTRANS, NOTRANS, &msub, &nY, &mScol, &rone, Usub, &msub,
        ...
        int ldR, double *work, int lwork)
{
    ...
    MPI_Allreduce(worktemp, Scol, s0*s, MPI_DOUBLE, MPI_SUM,
            MPI_COMM_WORLD);
    dlacpy_(ALL, &msub, &s, Xsub, &ldXsub, Usub, &msub, 1);
    dgemm_(NOTRANS, NOTRANS, &msub, &s, &s0, &rone, Usub, &msub, Scol,
